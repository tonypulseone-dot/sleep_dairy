...
- Перечисляй сны в том порядке, в каком они стоят на экране, сверху вниз.
- Ночной сон в списке дня, где сны идут по возрастанию времени (утро → вечер), если он стоит ПЕРВЫМ, перед утренними снами, — это ночь, которая закончилась утром этого дня и началась НАКАНУНЕ вечером: его "date" — предыдущий день. Если же список идёт от новых к старым (вечер сверху), ночь сверху началась в этот день.
...
- Длительность сна («9 часов 47 минут», «49 минут», «1ч 30м», «45м 18с») перепиши в "duration" как есть — это ОБЩАЯ длительность этого сна.
- Если у сна несколько чисел с пометками — ДС (дневной сон), НС (ночной сон), ВБ (бодрствование): в "duration" пиши общую длительность сна (у ночного — самое большое число без пометки или сумму ДС и НС), НЕ ВБ. ДС и НС у одной карточки — части одного сна, а не два сна.
- ВБ и числа между снами без блока сна («2 часа 55 минут» справа) — это бодрствование, НЕ сон. Не записывай их как сны.
- Цветные полоски на шкале часов (02, 04 … 22) без подписей времени — это график: "screen": "chart", "sleeps": []. НЕ оценивай время по положению полосок. Итоги под графиком («7 раз ночью», «10 ч 1 мин») — тоже не сны.
...
/**
 * Ночь, которая в списке дня стоит перед утренними снами, началась накануне.
 *
 * Трекеры по-разному относят ночь к дню: одни — к дню, когда уснули (и
 * ставят её в конец дня или сверху ленты «новое сверху»), другие — к дню,
 * когда проснулись (и ставят её первой, перед утренними снами, как PDF
 * Baby Soma). Если нейросеть дала такой ночи дату группы, а следом в той
 * же дате идут сны ПО ВОЗРАСТАНИЮ и все — после конца ночи, значит, ночь
 * закончилась этим утром: её начало — днём раньше. Список «от новых к
 * старым» идёт по убыванию, его правило не трогает.
 */
function fixMorningNights(sleeps: RecognizedSleep[]) {
  sleeps.forEach((night, index) => {
    if (!night.date || toMinutes(night.start) <= toMinutes(night.end)) return;
    const after: RecognizedSleep[] = [];
    for (const next of sleeps.slice(index + 1)) {
      if (next.date !== night.date) break;
      after.push(next);
    }
    if (after.length < 2) return;
    const ascending = after.every((item, i) => i === 0 || toMinutes(item.start) > toMinutes(after[i - 1].start));
    const afterWaking = after.every((item) => toMinutes(item.start) >= toMinutes(night.end));
    if (ascending && afterWaking) night.date = shift(night.date, -1);
  });
}
